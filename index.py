                ...
                if ligne % 2 == 0 and colonne % 2 == 0:
                    ...
                    if ligne > 5:
                        ...
                        if count_black_checkers < count_max_black_checkers:
                            Button(self.grid_frame, image=self.black_checkers, bg="peru", command=lambda row=ligne, column=colonne: self.get_button_coor(row, column)).grid(row=ligne, column=colonne)
                            count_black_checkers = count_black_checkers + 1
                            self.dict_button[ligne][colonne] = "black"
                        else:
                            Button(self.grid_frame, bg="peru", command=lambda row=ligne, column=colonne: self.get_button_coor(row, column)).grid(row=ligne, column=colonne)
                            self.dict_button[ligne][colonne] = "empty"
                elif ligne % 2 != 0 and colonne % 2 == 0:
                    if ligne > 5:
                        if count_white_checkers < count_max_white_checkers:
